...
fn main() {
    ...
    for manager in &managers {
        match Command::new("which")
            ...
            .status()
         {
            Ok(status) => if status.success() { installed_managers.push(*manager) },
            Err(_) => todo!()
        }
    }

    println!("Installed package managers: {}", installed_managers.join(", "));
    ...
    for manager in &installed_managers {
        match *manager {
            "apt" => {}
            "yay" => {
                match Command::new("yay").arg("-Ss").arg(package_name).output() {
                    Ok(output) => {
                        if !output.stdout.is_empty() {
                            ...
                            if package_name == name {
                                if line.contains("Installed") {
                                    results.push(format!("I -   yay: {} {} [installed]", fullname, version));
                                } else {
                ...
                match Command::new("cargo").arg("install").arg("--list").output() {
                    Ok(output) => {
                        if !output.stdout.is_empty() {
                            ...
                            let filtered_lines: Vec<&str> = lines.filter(|line| !line.starts_with(' ') && !line.is_empty()).collect();
                            for line in &filtered_lines {
                                ...
                                if package_name == name {
                                    results.push(format!("I - cargo: {} {} [installed]", name, version));
                                    installed = true;
                                }
                            }
                        }
                    }
                    Err(_) => results.push(format!("Error running 'install --list' for {}", manager)),
                }

                // check if availlable if not installed
                if !installed {
                    match Command::new("cargo").arg("search").arg(package_name).output() {
                        Ok(output) => {
                            if !output.stdout.is_empty() {
                                let stdout: Vec<u8> = output.stdout;
                                let stdout_string = String::from_utf8(stdout).unwrap();
                                ...
                                if package_name == name {
                                    results.push(format!("A - cargo: {} {} {}", name, version, description));
                                } else {
                                    results.push(format!("X - cargo: {} != {} {}", package_name, name, description));
                                }
                            }
                        }
